...
public class RoleServiceImpl implements RoleService {
    ...
    public Map<Integer, String> createRole(Role role) {
        ...
        Optional<Role> retrievedRole=roleMapper.getByName(role.getName());
    ...
    public Map<Integer, String> updateRole(Role role) {
        Map<Integer, String> message = new HashMap<>();
        Role retrievedRole = roleMapper.getById(role.getId());
        if (retrievedRole == null) {
            message.put(500, "Role with the given id doesnot exists!!!");
            return message;
        }
        if (!role.getName().isEmpty()) {
            Optional<Role> instantRole = roleMapper.getByName(role.getName().toUpperCase());//check whether the given role name already exists in the db
            if (instantRole != null) {
